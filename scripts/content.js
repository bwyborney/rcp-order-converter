...
function addButton() {
    // Find the header bar which contains the other download buttons
    const headerBar = document.getElementsByClassName('invoice-detail-header-bar')[0];
    const injectPoint = headerBar.children[1];

    // Create the download button element
    const downloadButton = document.createElement('div');
    downloadButton.innerHTML = "<a href = '#' class = 'print_pdf'><span><i style='font-style: normal; font-size: 1.5rem'>&#128202;</i> Download CSV</span></a>";
    downloadButton.onclick = () => orderConverter(); 
    downloadButton.style.marginRight = '1rem';
    downloadButton.style.marginLeft = '1rem';

    // Insert the download button
    injectPoint.insertAdjacentElement('afterend', downloadButton);
}
